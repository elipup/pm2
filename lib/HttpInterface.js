    ...
    Satan.executeRemote('getMonitorData', {}, function(err, data_proc) {
      var data = {
        ...
        processes: data_proc.map(function (p) {
          var process = JSON.parse(JSON.stringify((p))),
            env = p.pm2_env.env;

          process.pm2_env.env = {
            XT_PROCESS: env.XT_PROCESS,
            XT_NAME: env.XT_NAME,
            XT_VERSION: env.XT_VERSION,
            XT_DOMAIN: env.XT_DOMAIN,
            XT_URL: env.XT_URL,
            XT_PG_PORT: env.XT_PG_PORT,
            XT_SCHEDULE: env.XT_SCHEDULE,
